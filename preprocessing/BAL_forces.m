...
%                                       offset removed
% =========================================================================
function [BAL] = BAL_forces(BAL,BAL0,idxB,D,S,b,c,XmRefB,XmRefM,dAoA,dAoS,modelType,modelPos,testSec,groupNo,operManual,i)

%% Compute operating conditions
% get operating conditions 
oper = SUP_LTTgetOper_BAL(BAL,testSec);

if all(oper.dPb==0) && groupNo<=13
    oper.vInf = operManual.vInf{i};
    oper.rho = operManual.rhoInf{i}; 
    oper.tInf = operManual.tInf{i};
    oper.nu = (1.716e-5*(oper.tInf/273.15).^1.5*(273.15+110.4)./(oper.tInf+110.4))./oper.rho;
    oper.qInf = 0.5*oper.rho.*oper.vInf.^2;
end
